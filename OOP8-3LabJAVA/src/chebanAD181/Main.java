...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        //создание коллеции с элементами массива words
        ArrayList<String> list = new ArrayList<>(Arrays.asList(words));
        Collections.sort(list);//сортировка коллекции

        int identity = 1;//кол-во идентичных слов
        double frequency;//частота встречаемости слова
        for(int i = 0; i < list.size(); i++) {
            //обработка исключения с последним элементом коллекции
            if(i == list.size() - 2) {
                if(list.get(i).equals(list.get(i + 1))){
                    identity++;
                    frequency = identity / (double)list.size();
                    System.out.println("Слово: " + list.get(i) + ", Частота: " + frequency);
                    break;
                }
                else  {
                    frequency = identity / (double)list.size();
                    System.out.println("Слово: " + list.get(i + 1) + ", Частота: " + frequency);
                    break;
                }
            }
            if(list.get(i).equals(list.get(i + 1))) {
